...
/*
 * A voice manager session needs two things attached before it carries audio:
 * a vocproc, which is the device side, the microphone and the speaker, and a
 * stream, which is the side the modem talks to. Attaching only the vocproc
 * gives a session that starts without complaint and moves no audio in either
 * direction.
 */
