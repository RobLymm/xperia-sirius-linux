...
struct regmap *wcd9320_regmap_init_slimbus(struct slim_device *slimbus, struct regmap_config *config);
...
static int wcd9320_slim_probe(struct slim_device *slim)
{
	...
	wcd->regmap = wcd9320_regmap_init_slimbus(slim, &wcd9320_regmap_config);
				 ...
				 enum slim_device_status s)
{
	...
	/*
	 * The interface device has to be enumerated before anything is written
	 * to it: without a logical address its messages are misaddressed, the
	 * master NACKs them (NGD error interrupt, TX_NACKED_2), the port
	 * configuration never reaches the codec and the ADSP then refuses to
	 * start the AFE port. Mainline's wcd9335 does the same thing here.
	 */
	ret = slim_get_logical_addr(wcd->slim_ifd);
	if (ret) {
		dev_err(dev, "interface device has no logical address: %d\n", ret);
		return ret;
	}

	printk("slim PING");
	wcd->ifd_regmap = wcd9320_regmap_init_slimbus(wcd->slim_ifd, &wcd9320_ifd_regmap_config);
...
/* One module carries both halves: the SLIMbus device driver (this file) and
 * the codec platform driver it populates from the device tree (wcd9320.c). */
extern struct platform_driver wcd9320_codec_driver;
