...
static int resume(struct device *dev)
{
	...
	/*
	 * Take the controller back out of sleep mode.
	 *
	 * suspend() above calls set_suspend_mode() unconditionally, but the
	 * only caller of set_resume_mode() was a framebuffer blank notifier
	 * that is compiled out (#if 0 where CONFIG_FB used to be), so nothing
	 * ever woke the controller again. The symptom is a touchscreen that
	 * is still listed in /proc/bus/input/devices after resume, with its
	 * input device and interrupt intact, and completely unresponsive.
	 *
	 * After enable_irq(), because set_resume_mode() waits on reset_sem,
	 * which the interrupt handler is what releases.
	 */
	if (ts->is_suspended)
		set_resume_mode(ts);
...
static struct i2c_driver driver = {
		.probe = probe,
		/*
		 * shutdown() is a complete teardown -- it removes the sysfs
		 * entries, frees the interrupt, unregisters the input device,
		 * releases the GPIOs and the regulators -- and has the same
		 * signature as remove(). Without it wired up here the driver
		 * could be unbound but never freed anything, so the interrupt
		 * GPIO and the input device leaked and a second probe always
		 * failed with "GPIO request failed for max1187x_tirq". That
		 * left no way to recover the touchscreen short of a reboot.
		 */
		.remove = shutdown,
		.shutdown = shutdown,
		.id_table = id,
