...
| Audio | Mostly working | Speakers, earpiece, headphones, the handset microphone and call audio all work, and the radio app can switch between speaker, headphones and Bluetooth. Speakers are QDSP6 to Quaternary MI2S to two TFA9890 amplifiers; the earpiece is the top TFA9890. Headphones and microphones are a WCD9320 codec on SLIMbus with its 9.6 MHz master clock from the PM8941 divider on PMIC GPIO 15. Roughly every other capture returns silence, and the secondary microphone is not reading yet. `drivers/audio/wcd9320/`, `drivers/clk/pmic-clkdiv/` |
...
| Calls | Working | A call can be placed and answered, and carries audio in both directions: the far end hears the handset microphone and the caller's voice comes out of the phone. Voice audio goes through the DSP, and mainline has no driver for its voice services, so one is ported and adapted here. It needs no calibration data. One fault seen after a call: the modem's registration module can wedge, after which incoming calls are refused while ModemManager still reports the modem registered; a firmware restart clears it. See `docs/known-problems.md`. `drivers/audio/q6voice/` |
